...
export function Footer({ contact, resume }: FooterProps) {
  const currentYear = new Date().getFullYear();

  const handleEmailClick = () => {
    if (contact?.email) {
      navigator.clipboard.writeText(contact.email).then(() => {
        toast.success(`Alamat email (${contact.email}) berhasil disalin ke clipboard!`);
      }).catch(() => {
        // Fallback
      });
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-8">
          ...
          <div className="flex flex-col items-center md:items-end space-y-6">
            ...
            <Link
              href="#"
              className="flex items-center space-x-2 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors"
            >
