"use client";

import { Mail, Phone, Globe, Code } from 'lucide-react';
import { toast } from 'sonner';
import { AnimateOnScroll } from '@/components/ui/animate-on-scroll';
import { formatWhatsAppUrl } from '@/lib/utils/whatsapp';

interface ContactProps {
  contact: {
    email: string;
    phone: string | null;
    linkedin: string | null;
    github: string | null;
  } | null;
}

export function Contact({ contact }: ContactProps) {
  const email = contact?.email || "hello@example.com";

  const handleEmailClick = () => {
    if (email) {
      navigator.clipboard.writeText(email).then(() => {
        toast.success(`Alamat email (${email}) berhasil disalin ke clipboard!`);
      }).catch(() => {
        // Fallback
      });
    }
  };
  
  return (
    <section id="contact" className="py-12 sm:py-16 lg:py-20 px-6 bg-[var(--color-surface)]">
      <div className="max-w-7xl mx-auto text-center">
        <AnimateOnScroll>
          <div className="max-w-3xl mx-auto flex flex-col items-center">
            <span className="text-[var(--color-accent)] font-semibold tracking-wider uppercase text-sm mb-4 block">
              05 — CONTACT
            </span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-[var(--color-text-primary)]" style={{ fontFamily: 'var(--font-heading)' }}>
              Let&apos;s work together.
            </h2>
            <p className="text-lg text-[var(--color-text-secondary)] mb-10 max-w-xl">
              Open for freelance work and full-time roles. If you have a web development project, a data challenge to solve, or simply want to say hi, my inbox is always open!
            </p>
            
            <div className="flex flex-wrap justify-center gap-4">
              <a 
                href={`mailto:${email}`}
                target="_blank"
                rel="noopener noreferrer"
                onClick={handleEmailClick}
                className="flex items-center gap-2 px-6 py-3 rounded-full bg-[var(--color-background)] border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all text-[var(--color-text-primary)] font-medium"
              >
                <Mail className="w-5 h-5" />
                <span>Email Me</span>
              </a>
              
              {contact?.phone && (
                <a 
                  href={formatWhatsAppUrl(contact.phone)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-3 rounded-full bg-[var(--color-background)] border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all text-[var(--color-text-primary)] font-medium"
                >
                  <Phone className="w-5 h-5" />
                  <span>WhatsApp</span>
                </a>
              )}
              
              {contact?.linkedin && (
                <a 
                  href={contact.linkedin}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-3 rounded-full bg-[var(--color-background)] border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all text-[var(--color-text-primary)] font-medium"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
                  </svg>
                  <span>LinkedIn</span>
                </a>
              )}
              
              {contact?.github && (
                <a 
                  href={contact.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-3 rounded-full bg-[var(--color-background)] border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all text-[var(--color-text-primary)] font-medium"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8"/>
                  </svg>
                  <span>GitHub</span>
                </a>
              )}
            </div>
          </div>
        </AnimateOnScroll>
      </div>
    </section>
  );
}
