...
export function ProjectEditForm({ portfolio }: ProjectEditFormProps) {
  ...
  const form = useForm<ProjectFormValues>({
    ...
    defaultValues: {
      ...
      seoOgTitle: portfolio.seoOgTitle ?? "",
      seoOgDescription: portfolio.seoOgDescription ?? "",
      seoOgImage: portfolio.seoOgImage ?? "",
      seoCanonicalUrl: portfolio.seoCanonicalUrl ?? "",
      seoIndex: portfolio.seoIndex ?? true,
      seoFollow: portfolio.seoFollow ?? true,
    },
  });
  ...
  const onSubmit = (values: ProjectFormValues) => {
    ...
    startTransition(async () => {
      ...
      const result = await updateProject(portfolio.id, formData);
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
      ...
      {/* SEO SECTION */}
      <SeoFields form={form} pathPrefix="/projects/" entityId={portfolio.id} />
...
export const PortfolioEditForm = ProjectEditForm;
